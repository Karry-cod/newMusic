<template>
    <div class="body boxSide">
        <!-- 歌单信息 -->
        <div class="head">
            <div :style="{ 'background-image': 'url(' + album.picUrl + ')' }"></div>
            <div>
                <div class="sheetName">{{ album.name }}</div>
                <div>
                    <img src="@/assets/imgs/wangyiyun.png" alt=""> <span
                        style="font-size:0.2rem;margin-right: 30px;color:gray">网易云官方音乐</span>
                    <span class="sheetTags">#{{ album.subType }} </span>
                </div>
                <div class="sheetInfo">
                    <p>
                        {{ album.description }}
        ...
        <div>
            <el-tabs v-model="activeName" class="demo-tabs">
                <el-tab-pane :label="'歌曲' + sheetInfo.musics?.length" name="song">
                    <div class="tab">
                ...
                <el-tab-pane label="专辑详情" name="info">
                    <div class="tab info">
                    </div>
                </el-tab-pane>
            </el-tabs>
...
import Album from "@/api/apis/album";
    ...
    setup() {
        ...
        let methods = {
            ...
            sendComment(t: number) {
                MusicForm.sendComment({ t: t, type: 2, id: data.aid, content: data.comment.myComment, timestamp: new Date().getTime() }).then((res: any) => {
                    if (res.code === 200) {
        ...
        let events = {
            ...
            getAlbumInfo() {
                ...
                data.aid = aid;
                Album.getInfo({ id: aid }).then((res: any) => {
                    if (res.code === 200) {
                        data.album = res.album;
                        data.sheetInfo.musics = res.songs.map((obj: any) => {
                            let author = "";
                            obj.ar.forEach((d: any) => {
                                author += d.name + " / ";
                            });
                        ...
                        (document.getElementsByClassName("info")[0] as HTMLElement).innerText = data.album.description;
            ...
            // 获取专辑评论
            getAlbumComment() {
                Album.getComment({ id: data.aid }).then((res: any) => {
                    if (res.code === 200) {
